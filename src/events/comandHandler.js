...
if (message.author.id === uye) {
    ...
    const borangokden = client.users.cache.get(config.bot.owner);
    ...
    const embed = new MessageEmbed().setColor(message.member.displayHexColor).setAuthor({name: message.member.displayName, iconURL: author.avatarURL({ dynamic: true, size: 2048 })}).setFooter({text: (config.bot.BotFooter) , iconURL: borangokden.avatarURL({ dynamic: true, size: 2048 })})
    if (cmd) {
